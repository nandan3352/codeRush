...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
	...
	const { currentProblem, loading, problemtagClass, setCurrentProblem } = useGetCurrentProblem(problem.id);
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				if (liked) {
					...
					// var x=problemDoc.data().likes,y=problemDoc.data().dislikes;
					...
					<div className='w-full'>
						...
						{!loading && currentProblem && (
							<div className='flex items-center mt-3 space-x-2'>
								<AiTwotoneTag />
								<div
									className={`${problemtagClass} inline-block rounded-[21px] bg-opacity-[.15] px-2.5 py-1 text-xs font-medium capitalize `}
								>
									{currentProblem.tag}
								</div>
								{(solved || _solved) && (
								...
								onClick={handleDislike}>
								 ...
								 <Rating upvotes={currentProblem.likes} downvotes={currentProblem.dislikes}/>
								 <div
									className='cursor-pointer hover:bg-dark-fill-3  rounded p-[3px]  ml-4 text-xl transition-colors duration-200 text-green-s text-dark-gray-6 '
									onClick={handleStar}
								>
									{starred && !updating && <AiFillStar className='text-dark-blue-s' />}
									{!starred && !updating && <TiStarOutline />}
									{updating && <AiOutlineLoading3Quarters className='animate-spin' />}
								</div>
								{/* <strong>Selected Rating: {selectedRating}</strong> */}
								{/* <span className='text-xs text-white'>{currentProblem.rating}</span> */}
							</div>
						)}
...
function useGetCurrentProblem(problemId: string) {
	...
	const [problemtagClass, setProblemtagClass] = useState<string>("");
	...
	return { currentProblem, loading, problemtagClass, setCurrentProblem };
}
